"""
Moondream Object Detection Evaluation on YOLO Dataset
...
class MoondreamDetector:
    """Wrapper for moondream model to detect objects"""
    
    def __init__(self, class_mapping: Dict[str, int], max_objects: int = 100):
        """
        Args:
            class_mapping: Dict mapping class names to YOLO label IDs
                          e.g., {'person': 1, 'vehicle': 0, 'bicycle': 1}
            max_objects: Maximum number of objects to detect per class
        """
    ...
    def detect(self, image: Image.Image, confidence_threshold: float = 0.001) -> List[Detection]:
        """
        Run detection on an image for all classes in the mapping
        ...
        # Run detection for each class
        for class_name, yolo_class_id in self.class_mapping.items():
            try:
                result = self.model.detect(image, class_name, settings=settings)
                ...
                if "objects" in result:
                    for obj in result["objects"]:
                        ...
                        # Use a default confidence of 0.9 if not provided
                        # (moondream3 doesn't explicitly provide confidence scores)
                ...
                print(f"Warning: Detection failed for class '{class_name}': {e}")
                continue
...
class MoondreamYOLOEvaluator:
    ...
    def __init__(
        ...
        Args:
            ...
            max_objects_per_class: Max objects to detect per class
        """
        self.dataset = YOLODatasetLoader(images_path, labels_path)
        self.detector = MoondreamDetector(class_mapping, max_objects_per_class)
        self.class_mapping = class_mapping
...
def main():
    ...
    # Class mapping: model class names -> YOLO label IDs
    class_mapping = {
        'person': 0,
        'car': 1,
        'bicycle': 2,
    ...
    # Create evaluator
    evaluator = MoondreamYOLOEvaluator(
        images_path=images_path,
        labels_path=labels_path,
        class_mapping=class_mapping,
        class_names=class_names,
        max_objects_per_class=100
    )
